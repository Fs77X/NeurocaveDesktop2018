...
public class ConnectomeBuilder : MonoBehaviour {
    ...
    public void Build(List<Dictionary<string, string[][]>> connectomeList,string[] connectomeNames, bool isConnectomeReset, Dictionary<string, List<string>> connectomeTopologiesDictionary)
    {
        float n = connectomeList.Count;
        float[] anglearray = new float[connectomeList.Count];
        anglearray[0] = -(n - 1) / 2;
        //connectomeTopologiesDictionary = new Dictionary<string, string[]>();
        ...
        for (int i=0; i<connectomeList.Count; i++)
        {
            ...
            _connectomeParentList[i].AddComponent<SingleConnectome>().attachConnectomeData(connectomeList[i], connectomeNames[i], nodePrefab, edgePrefab, thresholdMax,thresholdMin, classificationMaterialDictionary, rotationAngle, isConnectomeReset, connectomeTopologiesDictionary);
